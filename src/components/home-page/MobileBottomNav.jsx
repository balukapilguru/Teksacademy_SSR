...
const flow = {
  ...
  rec_python: () => [
    botMessage("Full Stack Python or Java is a good fit. You will learn backend, frontend, APIs, and real-time projects.", {
      link: {
  ...
  rec_ds: () => [
    botMessage("Data Science is a strong choice. It covers Python, ML, Power BI, AI basics, and capstone projects.", {
      link: {
  ...
  rec_aws: () => [
    botMessage("AWS and DevOps is great for cloud roles. You will learn AWS, Docker, Kubernetes, Jenkins, and Terraform.", {
      link: {
  ...
  rec_dm: () => [
    botMessage("Digital Marketing is a practical path with SEO, Google Ads, Meta Ads, Analytics, and AI tools.", {
      link: {
  ...
  rec_bim: () => [
    botMessage("BIM is a good fit for AEC careers. You can learn Revit, Navisworks, and BIM coordination.", {
      link: {
  ...
  courses: () => [
    botMessage("You can browse all available courses with curriculum, fees, and duration.", {
  ...
  placements: () => [
    botMessage("Teks Academy offers placement support, hiring partner access, interview preparation, and career guidance.", {
...
const validateText = (values) => {
  ...
  else if (/\d/.test(values.name)) nextErrors.name = "Name should not contain numbers";
  ...
  if (!/^\d{10}$/.test(values.phone)) nextErrors.phone = "Enter a valid 10-digit mobile number";
    ...
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/55 p-3 sm:items-center">
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl">
        <button
          ...
          onClick={onClose}
          aria-label="Close"
          className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-full bg-gray-100 text-gray-700"
        >
          <X className="h-4 w-4" />
        </button>
...
function BrochureModal({ onClose }) {
  ...
  useEffect(() => {
    const handler = (event) => {
      if (courseBoxRef.current && !courseBoxRef.current.contains(event.target)) {
        ...
        <div className="relative" ref={courseBoxRef}>
          ...
          <div
            ...
            className={`flex w-full items-center justify-between rounded-lg border px-3 py-2 text-left text-sm outline-none ${
              errors.course ? "border-red-500 bg-red-50" : "border-gray-300 bg-white"
            }`}
          >
...
function ChatPanel({ onClose, onOpenBrochure }) {
  ...
  useEffect(() => {
    try {
      ...
      if (savedProfile) {
        ...
        setMessages(rawMessages ? JSON.parse(rawMessages) : [greet(parsedProfile.name)]);
      } else {
        setMessages([
          botMessage("Welcome to Teks Academy. Please share your details to start chat."),
        ]);
      }
    } catch {
      setMessages([
  ...
  const handleAction = (action) => {
    ...
    if (action.next === "brochure") {
      ...
      setMessages((prev) => [
        ...prev,
        botMessage("I opened the brochure form for you.", { actions: mainActions }),
      ]);
      return;
    }
    ...
    setTyping(true);
    setTimeout(() => {
      setMessages((prev) => [...prev, ...(flow[action.next]?.() || [greet(profile.name)])]);
      setTyping(false);
    }, 500);
  };
    ...
    <div className="fixed inset-x-0 bottom-0 z-[75] flex h-[100dvh] flex-col overflow-hidden bg-white text-gray-900 shadow-2xl lg:hidden">
      ...
      <div className="flex gap-2 border-t border-gray-200 bg-white p-2.5">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-green-50 py-2 text-xs font-semibold text-green-600"
        >
        ...
        <a
          href={PHONE_HREF}
          className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-blue-50 py-2 text-xs font-semibold text-blue-600"
        >
          ...
          <ul className="grid grid-cols-5">
            {items.map((item) => {
              ...
              const content = (
                <>
                  {item.highlight ? (
                    <span className="-mt-7 mb-0.5 grid h-12 w-12 place-items-center rounded-full bg-gradient-to-r from-[#FE543D] to-[#FF7854] text-white shadow-[0_0_15px_rgba(254,84,61,0.4)] ring-4 ring-white">
                      <Icon className="h-5 w-5" />
                    </span>
                  ) : (
                    <Icon className="h-5 w-5" />
                  )}
                  <span className={item.highlight ? "font-semibold text-[#FE543D]" : ""}>
                    {item.label}
                  </span>
                </>
              );
              ...
              return (
                <li key={item.label} className="relative">
                  {item.type === "link" ? (
                    <Link
                      href={item.href}
                      className="flex min-h-[58px] flex-col items-center justify-center gap-1 px-1 py-2.5 text-center text-[10px] font-medium leading-tight text-gray-600 transition-colors hover:text-[#FE543D]"
                    >
                      {content}
                    </Link>
                  ) : (
