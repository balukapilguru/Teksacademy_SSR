...
export default function GetDetailsModal({
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    if (selectedUniversity && show) {
      // Pre-fill the university field
      setFormData(prev => ({
        ...prev,
        university: selectedUniversity.universityName || selectedUniversity.university || "",
        universityId: selectedUniversity.sourceId || "",
        courseId: selectedUniversity.ProductId || "",
      }));
    }
  }, [selectedUniversity, show]);

  // Filter specialization options based on selected university
  useEffect(() => {
    if (courseName?.specializations) {
      // If we have a selected university, filter specializations
      if (selectedUniversity) {
        const filteredSpecializations = courseName.specializations.filter(spec =>
          spec.universities?.some(uni =>
            uni.universityName === selectedUniversity.universityName ||
            uni.sourceId === selectedUniversity.sourceId
          )
        );
        setSpecializationOptions(filteredSpecializations);
      } else {
        // Show all specializations if no university selected
  ...
  useEffect(() => {
    if (formData.specialization) {
      ...
      if (selectedSpec) {
        ...
        if (enableBrochureDownload && selectedSpec.universities) {
          const baseS3Url =
            "https://teksversity.s3.us-east-1.amazonaws.com/website/assets";

          const brochureMap = {};
          selectedSpec.universities.forEach((uni) => {
            if (uni.brochure) {
        ...
        // Reset university on specialization change
        setFormData((prev) => ({ ...prev, university: "" }));
      }
    } else {
      // Reset if no specialization selected
      setUniversityOptions([]);
      setBrochureUrls({});
    }
  }, [formData.specialization, specializationOptions, enableBrochureDownload]);
  ...
  return (
    ...
    >
      <div
        ...
        className={`bg-[#ffff] rounded-xl border
           relative shadow-2xl transform transition-all duration-500 ease-out flex md:flex-row flex-col ${show
            ? "scale-100 opacity-100 translate-y-0"
            : "scale-75 opacity-0 translate-y-8"
          } hover:shadow-3xl`}
      >
        <div className="hidden md:flex w-96 p-4 rounded-l-xl bg-[#2a619d] flex-col items-center justify-center text-white">
          <Image
        ...
        <div className="w-full p-4 overflow-y-auto flex flex-col justify-center">
          ...
          <form className="md:space-y-2" onSubmit={handleSubmit}>
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                className="mt-3 w-auto px-6 bg-[#2a619d] h-10 cursor-pointer text-white rounded-md font-semibold text-sm 
               hover:bg-white hover:text-[#2a619d]
               hover:border-2 hover:border-[#2a619d]"
              >
                Submit
              </button>
            </div>
