...
export const MobileOtpField = ({ value, onChange, onVerified, error }) => {
  ...
  const [canSendOtp, setCanSendOtp] = useState(true); // ✅ top Send OTP button control
  ...
  return (
    <div className="w-full relative pb-2 mt-3">
      {/* Label */}
      <label className="text-[0.7rem] text-black font-medium px-1 absolute left-2.5 -top-2.5 bg-white z-10">
      ...
      <div className="flex gap-2">
        ...
        {/* ✅ Top Send OTP button — disabled once OTP is sent (canSendOtp = false) */}
        {!isVerified && (
          <button
            type="button"
            onClick={handleSendOtp}
            disabled={!canSendOtp}
            className={`h-[46px] px-3 rounded-md text-xs font-normal
              ${
                !canSendOtp
                  ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                  : "bg-[#4a75a3] text-white"
              }`}
          >
            Send&nbsp;OTP
          </button>
        ...
        <div className="mt-3">
          ...
          <div className="flex gap-2 mt-2">
            ...
            <button
              ...
              className="flex-1 bg-[#4a75a3] text-white text-xs h-[40px] rounded-md"
            >
              Verify OTP
            </button>
          </div>
